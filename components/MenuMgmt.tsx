...
import { MenuItem, FoodType, AppSettings } from '../types.ts';
import { 
  Edit2, 
  Trash2, 
  Plus, 
...
const MenuMgmt: React.FC<MenuMgmtProps> = ({ menu = [], settings, onUpdate }) => {
  ...
  return (
    <div className={`rounded-2xl shadow-sm border overflow-hidden relative transition-all ${
      ...
      {isSyncing && (
        <div className={`absolute inset-0 z-50 flex flex-col items-center justify-center backdrop-blur-sm ${
          isDark ? 'bg-slate-950/70' : 'bg-white/60'
        }`}>
          <Loader2 className="w-8 h-8 text-blue-500 animate-spin mb-2" />
          <p className="text-xs font-bold text-blue-400 uppercase tracking-widest">Processing Catalog...</p>
        </div>
      )}
      ...
      <div className={`p-6 border-b flex flex-col md:flex-row md:items-center justify-between gap-4 ${
        ...
        <div>
          <h2 className={`text-xl font-bold ${isDark ? 'text-white' : 'text-gray-800'}`}>Menu Catalog</h2>
          <p className="text-xs text-gray-400 mt-1">Manage items and prices in your restaurant menu.</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative">
            ...
            <input 
              ...
              className={`pl-9 pr-4 py-2 border rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 w-full md:w-64 font-medium transition-all ${
                isDark 
          ...
          <button onClick={handleDownloadCsvTemplate} className={`flex items-center px-4 py-2 text-sm font-semibold rounded-xl transition-all shadow-sm active:scale-95 cursor-pointer ${
            isDark 
              ? 'text-emerald-300 bg-emerald-950/60 border border-emerald-900/60 hover:bg-emerald-900/50' 
              : 'text-emerald-700 bg-emerald-50 border border-emerald-200 hover:bg-emerald-100'
          }`} title="Download CSV Template for Menu Upload">
            <Download className="w-4 h-4 mr-2 text-emerald-500" /> CSV Template
          </button>
          <button onClick={() => fileInputRef.current?.click()} className={`flex items-center px-4 py-2 text-sm font-semibold rounded-xl transition-all shadow-sm active:scale-95 cursor-pointer ${
            isDark 
              ? 'text-slate-200 bg-slate-800 border border-slate-700 hover:bg-slate-750' 
              : 'text-gray-700 bg-white border border-gray-200 hover:bg-gray-50'
          }`}>
            <Upload className="w-4 h-4 mr-2 text-blue-500" /> Bulk Import
          </button>
          <button onClick={() => { setEditingItem(null); setIsModalOpen(true); }} className="flex items-center px-6 py-2 text-sm font-bold text-white bg-blue-600 rounded-xl hover:bg-blue-700 transition-all shadow-md active:scale-95 cursor-pointer">
            <Plus className="w-5 h-5 mr-1" /> Add Item
          </button>
        </div>
      </div>

      <div className={`px-6 py-3 border-b flex items-center text-[10px] font-bold uppercase tracking-wider ${
        isDark ? 'bg-blue-950/30 border-blue-900/40 text-blue-400' : 'bg-blue-50/50 border-blue-100 text-blue-600'
      }`}>
      ...
      <div className="overflow-x-auto min-h-[400px]">
        <table className="w-full text-left">
          <thead className={`text-[10px] uppercase font-bold tracking-widest border-b ${
          ...
          <tbody className={`divide-y ${isDark ? 'divide-slate-800' : 'divide-gray-50'}`}>
            {filtered.length === 0 ? (
              ...
              <tr key={item.id} className={isDark ? 'hover:bg-slate-800/40 transition-colors' : 'hover:bg-gray-50/50 transition-colors'}>
                <td className="px-6 py-4">
                  <div className={`font-bold text-sm ${isDark ? 'text-white' : 'text-gray-900'}`}>{item.name}</div>
                ...
                <td className={`px-6 py-4 font-bold text-sm ${isDark ? 'text-white' : 'text-gray-900'}`}>₹{item.price.toFixed(2)}</td>
                <td className="px-6 py-4">
                  <div className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold border ${
                    item.foodType === 'veg' 
                      ...
                      : isDark ? 'bg-rose-950/60 text-rose-400 border-rose-900/50' : 'bg-red-50 text-red-700 border-red-100'
                  }`}>
                ...
                <td className="px-6 py-4 text-right">
                  <div className="flex justify-end gap-2">
                    <button onClick={() => { setEditingItem(item); setIsModalOpen(true); }} className={`p-2 rounded-lg transition-all cursor-pointer ${
                      isDark ? 'text-slate-400 hover:text-blue-400 hover:bg-slate-800' : 'text-gray-400 hover:text-blue-600 hover:bg-blue-50'
                    }`} title="Edit Item">
                      <Edit2 className="w-4 h-4" />
                    </button>
                    <button onClick={() => handleDeleteClick(item)} className={`p-2 rounded-lg transition-all cursor-pointer ${
                      isDark ? 'text-slate-400 hover:text-rose-400 hover:bg-slate-800' : 'text-gray-400 hover:text-red-600 hover:bg-red-50'
                    }`} title="Delete Item">
        ...
        <div className="fixed inset-0 bg-black/60 z-[100] flex items-center justify-center p-4 backdrop-blur-sm">
          <div className={`rounded-2xl w-full max-w-md overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-200 border ${
            ...
            <div className={`p-6 border-b flex justify-between items-center ${
              ...
              <h3 className={`text-lg font-bold ${isDark ? 'text-white' : 'text-gray-800'}`}>{editingItem ? 'Edit Item' : 'Add New Item'}</h3>
            ...
            <form key={editingItem?.id || 'new'} onSubmit={handleSave} className="p-6 space-y-5">
              ...
              <div className="space-y-1">
                ...
                <input 
                  ...
                  className={`w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none font-bold uppercase transition-all ${
                    isDark ? 'bg-slate-800 border-slate-700 text-white placeholder-slate-500' : 'bg-gray-50 border-gray-200 text-gray-900'
                  }`} 
                />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <input 
                    ...
                    required 
                    className={`w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none font-black transition-all ${
                      isDark ? 'bg-slate-800 border-slate-700 text-white placeholder-slate-500' : 'bg-gray-50 border-gray-200 text-gray-900'
                    }`} 
                  />
                </div>
          ...
          <div className={`rounded-2xl w-full max-sm overflow-hidden shadow-2xl p-8 animate-in fade-in zoom-in duration-200 border ${
            ...
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-sm text-gray-400 mb-8 leading-relaxed">
                Are you sure you want to remove <span className={`font-bold ${isDark ? 'text-white' : 'text-gray-800'}`}>"{itemToDelete?.name}"</span> from the menu? This action cannot be undone.
              </p>
